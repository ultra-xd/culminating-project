...
class Player {
                    ...
                    */
                    ...
                    if (collisionType == HORIZONTAL || collisionType == DIAGONAL) { // check if collision is horizontal or diagonal
                        ...
                        if (totalVelocity.getY() > 0) { // check if player is moving up
                            newY = wallHitbox.y1 - this.height / 2; // set vertical player pos to the position directly on the bottom of the wall
                        } else if (totalVelocity.getY() < 0) { // check if player is moving down
                            newY = wallHitbox.y2 + this.height / 2 // set vertical player pos to the position directly on the top of the wall
                        }
                    ...
                    if (collisionType == VERTICAL || collisionType == DIAGONAL) { // check if collision is vertical or diagonal
                        ...
                        if (totalVelocity.getX() > 0) { // check if player is moving right
                            newX = wallHitbox.x1 - this.width / 2; // set horizontal player pos to the position directly on the left of the wall
                        } else if (totalVelocity.getX() < 0) { // check if the player is moving left
                            newX = wallHitbox.x2 + this.width / 2 // set horizontal player pos to the position directly on the right of the wall
                        }
    ...
    getHitbox() {
        return new Rectangle(this.position.getX() - this.width / 2, this.position.getY() - this.height / 2, this.width, this.height); // return a rectangle object defining the hitbox of the player
    }
